          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden">
            ...
            <div className="p-8">
              ...
              <div className="flex gap-4 mt-8">
                <button onClick={() => router.push("/orders")} className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-lg shadow-indigo-100">View Orders</button>
                <button onClick={() => window.print()} className="px-6 py-4 bg-gray-100 text-gray-600 rounded-2xl font-bold">Print</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        {items.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed">
            <button onClick={() => router.push("/products")} className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold">Shop Now</button>
          </div>
        ) : (
          <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div key={item.id} className="bg-white p-4 rounded-2xl flex items-center gap-6 border border-gray-100 shadow-sm">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      <button onClick={() => updateQty(item.id, -1)} className="w-8 h-8 rounded-lg bg-gray-100 font-bold hover:bg-gray-200 transition-colors">-</button>
                  ...
                  <div className="text-right">
                    ...
                    <button onClick={() => removeItem(item.id)} className="text-xs text-rose-500 font-bold mt-2 hover:underline">Remove</button>
                  </div>
                </div>
              ))}
            </div>
            <div className="bg-white p-6 rounded-3xl h-fit shadow-xl border border-gray-50">
              ...
              <button onClick={handleCheckout} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-lg shadow-indigo-100 active:scale-95 transition-all">Checkout</button>
            </div>
          </div>
        )}
      </div>
